...
module.exports = {
  ...
  getCertificateInfo: function (cert, cb) {
    var infoObject = {},
  	...
  	var openssl = spawn('openssl', ['x509', '-noout', '-issuer', '-subject', '-dates', '-nameopt', 'RFC2253', '-ext', 'subjectAltName']);
    ...
    openssl.on('close', function (code) {
      ...
      // Check if output is exact four lines and does not contain "X509v3 Subject Alternative Name: "
      if (lineArray.length !== 4 && !lineArray.includes('X509v3 Subject Alternative Name: ')) {
      ...
      // X509v3 Subject Alternative Name information
      let X509v3_Elements = lineArray.splice(lineArray.indexOf('X509v3 Subject Alternative Name: ')+1,1)
      infoObject.X509v3 = X509v3_Elements[0].replace('DNS:', '').split(',').map(item => item.trim());
